...
def clean_and_prepare_data(df_2009, df_2010):
    ...
    print(
        f"Guest checkout rows retained: "
        f"{(~df_clean['Is_Registered']).sum():,}"
    )
...
def make_recommendations(df_clean, df_cancelled):
    ...
    guest_revenue = df_clean.loc[
        ~df_clean['Is_Registered'], 'Revenue'
    ].sum()
    ...
    return_rate = (
        len(df_cancelled) /
        (len(df_clean) + len(df_cancelled)) *
        100
    )

    print(
        f"1. Guest checkout conversion: Guest sales generated "
        f"${guest_revenue:,.2f} "
        f"({guest_share:.1f}% of clean revenue). "
        "Offer a registration incentive after checkout to capture "
        "customer information for follow-up marketing."
    )

    print(
        f"2. Return monitoring: {len(df_cancelled):,} rows "
        f"({return_rate:.1f}% of all raw transactions) "
        "were cancellations or returns. Review frequent returns "
        "to identify potential product or fulfillment problems."
    )

    print(
        f"3. Revenue protection: Clean sales total "
        f"${total_revenue:,.2f}. "
        "Keep the positive-quantity and positive-price checks in "
